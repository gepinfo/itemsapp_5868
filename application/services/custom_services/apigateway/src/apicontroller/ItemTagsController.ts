...
public (req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into ItemTagsController.ts: ');
        new ApiAdapter().(Constant.GEPITEMTAGSMANAGERURL + `` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
            new CustomLogger().showLogger('info', 'Exit from ItemTagsController.ts: ');
        }).catch(err => {
            res.send(err);
        });
    }
